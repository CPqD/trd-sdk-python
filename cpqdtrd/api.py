...
class TranscriptionApi:
    ...
    def audiofile_list(self, batch: str = ""):
        if batch:
            return requests.get(
                "{}/audiofile/list/batch/{}".format(self._uri, batch), auth=self._auth
            )
        else:
            return requests.get("{}/audiofile/list/".format(self._uri), auth=self._auth)

    def audiofile_get(self, uid: str):
        return requests.get("{}/audiofile/{}".format(self._uri, uid), auth=self._auth)

    def audiofile_create(self, file_name: str):
        return requests.get(
            "{}/audiofile/create/{}".format(self._uri, file_name), auth=self._auth
        )

    def audiofile_create_batch(self, batch: str):
        return requests.get(
            "{}/audiofile/create/batch/{}".format(self._uri, batch), auth=self._auth
        )

    def audiofile_upload(self, file_path: str, batch: str = ""):
        upload_request = "{}/audiofile/upload/".format(self._uri)
        if batch:
            data = {"batch": batch}
        else:
            data = {}
    ...
    def audiofile_delete(self, uid: str, delete_on_disk: bool = False):
        ...
        return requests.delete(delete_request.format(self._uri, uid), auth=self._auth)

    def audiofile_delete_batch(self, batch: str, delete_on_disk: bool = False):
        delete_request = "{}/audiofile/delete/batch/{}"
        if delete_on_disk:
            delete_request += "?deleteOnDisk=true"
        return requests.delete(delete_request.format(self._uri, batch), auth=self._auth)
    ...
    def transcription_start(self, audio_id: str, request_args: dict = {}):
        ...
        return requests.get(start_request.format(self._uri, audio_id), auth=self._auth)

    def transcription_start_batch(
        self, batch: str, word_hints: str = "", lm_uri: str = ""
    ):
        ...
        if lm_uri:
            start_request += sep
            sep = "&"
            start_request += "lm.uri={}".format(lm_uri)
        if word_hints:
            start_request += sep
            start_request += "hints.words={}".format(word_hints)
        return requests.get(start_request.format(self._uri, batch), auth=self._auth)

    def transcription_status(self, audio_id: str):
        return requests.get(
            "{}/transcription/status/audiofile/{}".format(self._uri, audio_id),
            auth=self._auth,
        )

    def transcription_status_batch(self, batch: str):
        return requests.get(
            "{}/transcription/status/batch/{}".format(self._uri, batch), auth=self._auth
        )
    ...
    def transcription_reset(self, audio_id: str, hard: bool = False):
        ...
        return requests.get(reset_request.format(self._uri, audio_id), auth=self._auth)

    def transcription_reset_batch(self, batch: str, hard: bool = False):
        reset_request = "{}/transcription/reset/batch/{}"
        if hard:
            reset_request += "?hard=true"
        return requests.get(reset_request.format(self._uri, batch), auth=self._auth)
    ...
    def transcription_result_batch(self, batch: str, format: str = ""):
        result_request = "{}/transcription/result/batch/{}".format(self._uri, batch)
        if format != "":
            result_request += "?format=" + format
        return requests.get(result_request, auth=self._auth)

    def query_collection(self, collection: str, query: dict, project: list = []):
        query_request = "{}/query/collection/{}".format(self._uri, collection)
        sep = "?"
        for k in query:
